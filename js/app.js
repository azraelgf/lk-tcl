(() => {
    ...
    class Popup {
        ...
        eventsPopup() {
            document.addEventListener("click", function(e) {
                ...
                if (buttonClose) {
                    e.preventDefault();
                    const popupToClose = buttonClose.closest(`.${this.options.classes.popup}`);
                    if (popupToClose) this.close(`#${popupToClose.id}`);
                    return;
                }
                if (!e.target.closest(`.${this.options.classes.popupContent}`)) {
                    const popupToClose = e.target.closest(`.${this.options.classes.popup}`);
                    if (popupToClose) this.close(`#${popupToClose.id}`);
                }
            }.bind(this));
            document.addEventListener("keydown", function(e) {
                ...
                if (this.options.focusCatch && e.key === "Tab") this._focusCatch(e);
            }.bind(this));
        ...
        open(selectorValue) {
            if (bodyLockStatus) {
                ...
                if (this.openedPopups.includes(this.targetOpen.selector)) return;
                ...
                this.targetOpen.element.style.zIndex = 1e3 + this.openedPopups.length;
                this.previousOpen.selector = this.targetOpen.selector;
                this.previousOpen.element = this.targetOpen.element;
                this._selectorOpen = false;
                this.openedPopups.push(this.targetOpen.selector);
                if (this.openedPopups.length === 1) {
        ...
        close(selectorValue) {
            const selector = selectorValue || this.previousOpen.selector;
            ...
            if (this.youTubeCode) {
                const videoElem = popupElement.querySelector(`[${this.options.youtubePlaceAttribute}]`);
            ...
            this.openedPopups = this.openedPopups.filter((sel => sel !== selector));
            if (this.openedPopups.length === 0) {
                document.documentElement.classList.remove(this.options.classes.bodyActive);
                !this.bodyLock ? bodyUnlock() : null;
            }
            this._removeHash();
            this.lastClosed.selector = selector;
        ...
        _focusTrap() {
            ...
            if (focusable.length > 0) if (this.lastFocusEl) this.lastFocusEl.focus(); else focusable[0].focus();
        }
        _loadYouTube() {
    ...
    function initSliders() {
        if (document.querySelector(".guid-single__body")) new Swiper(".guid-single__body", {
        ...
        if (document.querySelector(".product-main") && document.querySelector(".product-thumbs")) {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        const quizSteps = document.querySelectorAll(".quiz-order__content > .quiz-order__body");
        const nextBtns = document.querySelectorAll(".item-quiz-add__next");
        const prevBtns = document.querySelectorAll(".item-quiz-add__prev");
        let currentStep = 0;
        function goToStep(index, isInit = false) {
            if (index < 0 || index >= quizSteps.length) return;
            quizSteps.forEach(((step, i) => {
                step.classList.toggle("_active", i === index);
            }));
            progressItems.forEach(((item, i) => {
                if (i <= index) item.classList.add("_active");
            }));
            if (index < currentStep) progressItems[currentStep]?.classList.remove("_active");
            currentStep = index;
            const progress = document.querySelector(".progress");
            if (progress) progress.classList.toggle("progress_active", currentStep >= 1);
            if (!isInit) progressItems[index]?.scrollIntoView({
                behavior: "smooth",
                inline: "center"
            });
        }
        ...
        prevBtns.forEach((btn => {
            btn.addEventListener("click", (() => {
                if (progressItems[currentStep]) progressItems[currentStep].classList.remove("_active");
                goToStep(currentStep - 1);
            }));
        }));
        progressItems.forEach(((item, index) => {
            item.addEventListener("click", (() => {
                if (index < currentStep) for (let i = currentStep; i > index; i--) progressItems[i]?.classList.remove("_active");
                goToStep(index);
            }));
        }));
        goToStep(0, true);
        ...
        if (cargoBlock) {
            ...
            const bodies = cargoBlock.querySelectorAll(".cargo__body");
            let currentIndex = 0;
            const checked = cargoBlock.querySelector('input[name="cargo"]:checked');
            if (checked) {
                currentIndex = Number(checked.value) - 1;
                bodies[currentIndex].classList.add("_active");
            }
            function updateCargoBody(value) {
                const newIndex = value - 1;
                if (newIndex === currentIndex) return;
                bodies[currentIndex].classList.remove("_active");
                bodies[newIndex].classList.add("_active");
                currentIndex = newIndex;
            }
            radios.forEach((radio => {
                radio.addEventListener("change", (() => {
                    if (radio.checked) updateCargoBody(Number(radio.value));
                }));
            }));
        }
    }));
